...
import { AssetImage, Card, Chip, ErrorState, PageHeader, Section, Skeleton, V1Link } from "@/components/admin-v2/primitives";
  ...
  return (
    <>
      ...
      */}
      ...
      >
        {/*
          The shelf takes a dropped folder too. Which shelf it lands on follows
          the filter the operator is already looking at: dropping while
          "Concept" is selected files it as inspiration, because that is what
        ...
        <DropZone
          ...
          label={
            designFilter === "ready"
              ...
              : `Add inspiration for ${entity.name}`
          }
        >
          <DesignShelf
            entityId={entity.id}
            ...
            return (
              <div key={p.id} className="ax-card overflow-hidden">
                ...
                <div className="p-2.5">
                  ...
                  <div className="mt-1 flex items-center justify-between text-[11px]">
                    <span className="tabular-nums text-[hsl(var(--ax-secondary))]">{fmtMoney(p.price)}</span>
                    <Link to={productHref(p.id)} className="text-[hsl(var(--ax-faint))] hover:text-[hsl(var(--ax-ink))]">
                      Edit
                    </Link>
                  </div>
                  {store && (
